...
def edit_account(id):
    ...
    if form.validate_on_submit():
        if current_user == user and current_user.role.permission == 'Administrator':
            user.name = form.name.data
            user.real_name = form.real_name.data
            user.position = form.position.data
            user.role.permission = form.permission.data
            if form.old_password is not None and form.new_password is not None and form.confirm_password is not None:
                if user.verity_password(form.old_password) and form.new_password == form.confirm_password:
                    user.password=form.new_password
            db.session.add(user)
            db.session.commit()
            return redirect(url_for('main.account_manage'))
        elif current_user.role.permission=='Administrator':
            user.name = form.name.data
            user.real_name = form.real_name.data
            user.position = form.position.data
            user.role.permission = form.permission.data
